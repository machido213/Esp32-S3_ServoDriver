...
這是一個基於 **ESP32-S3 (N16R8)** 的多軸伺服馬達驅動與工控系統。專為 ROS 機器人下位機設計，具備 7 軸脈波控制、工控箱實體按鈕整合、安全互鎖邏輯，以及支援 **WiFi 斷線救援 (AP Mode)** 與 **參數記憶 (NVS)** 的暗黑風格 Web 控制儀表板。

## ✨ 專案特點 (Features)

*   **核心控制**: 支援 7 軸步進/伺服馬達控制 (Pulse/Direction 模式)。
*   **Web 儀表板**: 內建暗黑工業風 Web Server (SPIFFS 託管)，支援 RWD 響應式設計。
    *   **即時監控**: IO 狀態 (停止按鈕、接觸器回饋)、環境溫度。
    *   **馬達控制**: 支援單次定位控制 (Steps/Hz) 與 **長按點動 (Jog) 控制** (正/反轉)。
    *   **系統控制**: 遠端啟動、強制斷電、伺服始能切換。
*   **智慧網路管理**:
    *   **NVS 記憶**: 自動儲存 WiFi SSID、密碼與固定 IP 設定，斷電不遺失。
    *   **斷線救援 (AP Mode)**: 若連線失敗 (如密碼錯誤或路由器斷電)，自動切換至熱點模式 (`ESP32-Rescue-Mode`)，允許使用者連入重新設定。
*   **安全邏輯**:
...
### 2. 救援模式 (AP Mode)
若 ESP32 連接 WiFi 失敗 (超過 5 次重試)，將自動切換為 **熱點模式**：
1.  **搜尋 WiFi**: `ESP32-Rescue-Mode`
2.  **連接密碼**: 無 (或自訂)
3.  **瀏覽器訪問**: `http://192.168.4.1`
4.  在儀表板的「網路設定」區塊輸入正確的 WiFi 資訊，點擊「儲存並重啟」即可。

---

## 🖥️ Web 儀表板 (Dashboard)

系統啟動後，請訪問設定的 IP (預設 `http://192.168.2.124`)。

### 功能區塊
1.  **Monitor (左側)**: 監控停止按鈕、接觸器狀態、機箱溫度。
2.  **Network Config (中間)**: 修改 WiFi SSID、密碼與固定 IP (修改後自動重啟)。
3.  **Control (中間)**: 遠端執行啟動、斷電、伺服始能切換。
4.  **OTA (右側)**: 填入 `.bin` 連結進行無線更新。
5.  **Motors (下方)**: 7 軸馬達控制，具備 **RUN (單次執行)** 與 **Jog (點動長按)** 按鈕。

---

## 🚀 開發與燒錄 (Development)

本專案使用 ESP-IDF v5.4 開發，並採用 SPIFFS 存放網頁檔案。

**專案結構要求:**
必須在根目錄建立 `spiffs_image` 資料夾，並將 `index.html` 放入其中。

Esp32-S3_ServoDriver/
├── CMakeLists.txt (Root, 需包含 spiffs_create_partition_image)
├── partitions.csv (需定義 storage 分區)
├── spiffs_image/
│ └── index.html
└── main/
├── main.c
└── CMakeLists.txt (Main, 移除 index.html 引用)
**編譯環境設定:**
```bash
...
```
編譯與燒錄:
由於使用了 SPIFFS 打包，請確保 CMake 已正確配置 spiffs_create_partition_image。
# 清除舊的編譯檔 (建議)
idf.py fullclean

# 編譯、燒錄 (包含程式碼與網頁檔)、開啟監控
idf.py build flash monitor

沒問題，我將整份 README.md 的原始碼放在同一個程式碼區塊中，你直接點擊右上角的「複製」按鈕，然後貼到你的 README.md 檔案裡即可。
code
Markdown
# 🤖 ESP32-S3 7-Axis Servo Driver & Industrial Controller
...
idf.py build flash monitor
⚠️ 注意事項:
1.  PSRAM: 必須開啟 Octal PSRAM，否則 N16R8 模組將無法正常運作。
2.  Partition Table: 必須預留足夠空間給 storage 分區以存放網頁。